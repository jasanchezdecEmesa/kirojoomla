...
class JFormFieldapikey extends FormField {
	...
	protected function getInput()
	{
		...
		// load js
		Factory::getDocument()->addScript(Uri::root(true).'/media/plg_system_web357framework/js/admin.min.js?v=20240527015748');
		...
		HTMLHelper::_('script', 'system/html5fallback.js', ['version' => 'auto', 'relative' => true]);
		...
		if (!empty($value))
		{
			...
			else 
			{
				...
				{
					$html .= '<div style="margin: 20px 0;" id="w357-activated-successfully-msg" class="alert alert-success"><span class="icon-save"></span> '.Text::_('Your API Key <strong>' . $value . '</strong> is active and validated.').'</div>';
					$show_active_key_button = false;
				}
				elseif (isset($resp->status) && $resp->status == 0)
				{
					$html .= '<div style="margin: 20px 0;" id="w357-activated-successfully-msg" class="alert alert-danger"><span class="icon-cancel"></span> '.Text::_('Your API Key <strong>' . $value . '</strong> is valid, but is not activated yet.<br>Click the button below to activate it.').'</div>';
				}
				...
				{
					$html .= '<div style="margin: 20px 0;" id="w357-activated-successfully-msg" class="alert alert-danger"><span class="icon-cancel"></span> '.Text::_('Call with Web357\'s License Manager has been failed. <br>Please, try again later or contact us at support@web357.com.').'</div>';
				}
			}
			...
			if ($show_active_key_button)
			{
				...
				$html .= '<p><a class="btn btn-success web357-activate-api-key-btn"><strong>'.Text::_('Activate Api Key').'</strong></a></p>';
			}
