...
function guardarApertura(id){
    ...
    datos.hora_inicio = jQuery("#formulario #hora_inicio").val();
    datos.hora_final = jQuery("#formulario #hora_final").val();
    datos.id_tipo_visita = jQuery('#formulario #id_tipo_visita').val();
    datos.idioma = jQuery('#formulario #idioma').val();

    var dias = jQuery('input[name="dias_semana"]:checked');
    var coma = false;
    datos.dias_semana = '';
    dias.each(function(){
        if(coma) datos.dias_semana +=',';
        datos.dias_semana += this.value;
        coma = true;
    });
    ...
    if (
        ...
        datos.hora_inicio.length < 1
    ) {
        alert('Por favor, rellene al menos los siguientes campos: Día inicio, Día fin y hora inicio.');
        return;
    }

    //Llamada Ajax
    var url =comprobarLocalhost()+ "/index.php?option=com_ereservas&task=aperturas.comprobarid&format=json";
...
function reinicioapertura(){
    ...
    var codigo='';
    jQuery.ajax({
        url: '/index.php?option=com_ereservas&task=aperturas.reset&format=json',
        type: 'POST',
        success: function (data) {
...
function modal_editar(id){
    ...
    datos.id=id;
    jQuery.ajax({
        url:comprobarLocalhost()+ '/index.php?option=com_ereservas&task=aperturas.editar&format=json',
        type: "POST",
        data: datos,
        success: function(data) {
            ...
            jQuery("#formulario #hora_inicio").val(apertura.hora_inicio);
            jQuery("#formulario #hora_final").val(apertura.hora_final);
            ...
            jQuery('#formulario #idioma').chosen();
            jQuery('#formulario #idioma').val(apertura.idioma);
            jQuery('#formulario #id_tipo_visita').trigger("liszt:updated");

            var dias =apertura.dias_semana.split(',');
            for(i=0;i<dias.length;i++){
                jQuery('#dia-'+dias[i]).prop('checked', true);
            }
...
function modal_eliminar(id){
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            jQuery("#modal-eliminar #borrar-hora-inicio").text(apertura.hora_inicio.substring(0,5));
...
function borrarApertura(id){
    ...
    datos.id=id;
    jQuery.ajax({
       url:comprobarLocalhost()+ "/index.php?option=com_ereservas&task=aperturas.borrarApertura&format=json",
        type: "POST",
        data: datos,
        success: function (data) {
