 ...
 * @since  1.6
 */
class EreservasViewVisitaspublicas extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		$this->items = $this->obtenerVisitas();
	...
	public function obtenerVisitas(){
        ...
        $query->select($db->quoteName(array('id','imagen','nombre','precio','presentacion','descripcion','nombre_en','presentacion_en','descripcion_en', 'alias', 'alias_en')));
        $query->from($db->quoteName('#__ereservas_tipo_visita'));
        $query->where($db->quoteName('imagen') . ' != ""');
