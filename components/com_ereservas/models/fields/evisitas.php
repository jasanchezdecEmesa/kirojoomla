 ...
 * @since  1.6
 */
class JFormFieldEVisitas extends FormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		$query
			->select(
				array(
					'a.*',
					'a.'.$fk_value,
						$db->quoteName('b.nombre'),

				)
			)
			->from($this->table.' as a')
			//->from($db->quoteName($this->table,'a'))
		->innerJoin($db->quoteName('#__ereservas_tipo_visita','b').' ON (' . $db->quoteName('a.id_tipo_visita') . ' = ' . $db->quoteName('b.id') . ')');
        if ($this->hideTrashed)
        {
		...
		switch ($this->input_type)
		{
			...
			default:
				...
				{
					$options[] = JHtml::_('select.option', $result->{$this->key_field}, $result->nombre.' - '.date('d-m-Y', strtotime($result->fecha)).' - '.$result->hora_inicio);
				}
