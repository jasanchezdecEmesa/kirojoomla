 ...
 * @since  1.6
 */
class EreservasModelExpertis extends \Joomla\CMS\MVC\Model\ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
        ...
        // Join over the foreign key 'id_idioma'
        $query->select(array('`b`.`id` AS id_factura', '`b`.`nif` AS factura_nif','`b`.`direccion` AS factura_direccion'  ));
        $query->join('LEFT', '#__ereservas_factura AS b ON b.`id_reserva` = a.`id`');
		...
		$query->where('a.estado = "confirmado"');
            ...
            if (!empty($search))
            {
                ...
                else
                {
					...
					$query->where('( a.nombre LIKE ' . $search . '  OR  a.telefono LIKE ' . $search . '  OR  a.email LIKE ' . $search . '  OR  a.cpostal LIKE ' . $search . ' OR  a.id LIKE ' . $search . ' )');
                }
            }
...
//						->select('CONCAT(`#__ereservas_visita_3206109`.`nombre`, \'  - \', `#__ereservas_visita_3206109`.`fecha`, \'  - \', `#__ereservas_visita_3206109`.`hora_inicio`) AS `fk_value`')
                        ->select('CONCAT(`#__ereservas_visita_3206109`.`fecha`, \'  - \', `#__ereservas_visita_3206109`.`hora_inicio`) AS `fk_value`')
			...
			$item->asistencia = JText::_('COM_ERESERVAS_RESERVAS_ASISTENCIA_OPTION_' . strtoupper($item->asistencia));
		}
