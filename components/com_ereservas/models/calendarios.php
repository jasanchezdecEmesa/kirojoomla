...
class EreservasModelCalendarios extends \Joomla\CMS\MVC\Model\ListModel
{
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->frecuencia = JText::_('COM_ERESERVAS_CALENDARIOS_FRECUENCIA_OPTION_' . strtoupper($item->frecuencia));

				// Get the title of every option selected.
				$options      = explode(',',$item->dias_semana);
				$options_text = array();
				...
				$item->dias_semana = !empty($options_text) ? implode(',', $options_text) : $item->dias_semana;

				// Get the title of every option selected.
				$options      = explode(',',$item->dias_mes);
				$options_text = array();

				foreach ((array) $options as $option)
				{
					$options_text[] = JText::_('COM_ERESERVAS_CALENDARIOS_DIAS_MES_OPTION_' . strtoupper($option));
				}

				$item->dias_mes = !empty($options_text) ? implode(',', $options_text) : $item->dias_mes;
		}
