...
class SppagebuilderAddonForm_builder extends SppagebuilderAddons
{
    ...
    public static function getAjax()
    {
        ...
        if ($mail->Send()) {
            ...
            if($inputs[6]["value"] === "1565611854741"){
                $datos = new stdClass();
                $datos->ordering=1;
                $datos->state=1;
                $datos->checked_out=1;
                $datos->checked_out_time='now()';
                $datos->created_by=0;
                $datos->modified_by=0;
                $datos->nombre= $inputs[0]["value"];
                $datos->email=$inputs[1]["value"];
                $datos->plazas=$inputs[2]["value"];
                $datos->turno=null;
                $datos->telefono=null;
                $datos->fecha=$inputs[3]["value"];
                JFactory::getDbo()->insertObject('#__ereservas_lista_espera', $datos);
            }
